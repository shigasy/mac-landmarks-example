...
struct NavigationDetail: View {
    ...
    // 引数から渡されたlandmarkDataのidと一致するindexを返す。配列とかで使う
                ...
                .overlay(
                    GeometryReader { proxy in
                        Button("Open in Maps") {
                            ...
                            // proxy.size.widthはgeometryReaderで渡された親のサイズのwidth
                            // そのオフセットで内側になる
                            // webのcssならrelationとabsolute, top, rightで表現するなあ
                            // ::afterとか使って
...
struct NavigationDetail_Previews: PreviewProvider {
    static var previews: some View {
        NavigationDetail(landmark: landmarkData[0]).environmentObject(UserData())
    }
}
